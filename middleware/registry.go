...
import (
	...
	configv1 "gateway-service/api/gateway/config/v1"
	...
	if method, ok := p.getMiddleware(createFullName(cfg.Name)); ok {
		if cfg.Required {
			...
			if err != nil {
				...
				log.Errorw(klog.DefaultMessageKey, "Failed to create required middleware", "reason", "create_required_middleware_failed", "name", cfg.Name, "error", err, "config", cfg)
				return nil, err
			}
			return instance, nil
		}
		...
		if err != nil {
			...
			log.Errorw(klog.DefaultMessageKey, "Failed to create optional middleware", "reason", "create_optional_middleware_failed", "name", cfg.Name, "error", err, "config", cfg)
